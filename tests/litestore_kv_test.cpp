...
{
    ASSERT_LS_OK(litestore_save_kv(ctx, key.c_str(), key.length(),
                                   dataIter.getIter()));
...
TEST_F(LitestoreKVTest, get_kv_retrieves_data)
{
    ASSERT_LS_OK(litestore_save_kv(ctx, key.c_str(), key.length(),
                                   dataIter.getIter()));
    iter::StrMap result;
    ASSERT_LS_OK(litestore_get_kv(ctx, key.c_str(), key.length(),
                                  &buildMap, &result));
...
{
    ASSERT_LS_OK(litestore_save_null(ctx, key.c_str(), key.length()));
    iter::StrMap result;
    EXPECT_LS_ERR(litestore_get_kv(ctx, key.c_str(), key.length(),
                                   &buildMap, &result));
}
...
TEST_F(LitestoreKVTest, kv_to_null_to_kv_to_raw_to_kv)
{
    ...
    ASSERT_LS_OK(litestore_update_kv(ctx, key.c_str(), key.length(),
                                     dataIter.getIter()));
    EXPECT_EQ(3, readObjects()[0].type);  // LS_KV
